...
public class TranslatableContentsMixin {
    ...
    private String incognito$interceptTranslation(Language instance, String key, Operation<String> original) {
        // Vanilla translation key - always allow (no exploit risk)
        if (ModTracker.isVanillaTranslationKey(key)) {
            return original.call(instance, key);
        }
        
        // Server resource pack key - allow (prevents anti-spoof detection)
        if (ModTracker.isServerPackTranslationKey(key)) {
            return original.call(instance, key);
        }
    ...
    private String incognito$interceptTranslationWithFallback(
