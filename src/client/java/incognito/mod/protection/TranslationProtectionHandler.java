 ...
 * 
 * Alert format:
 * ⛔ [Incognito] Translation exploit detected!     (header with cooldown)
...
public class TranslationProtectionHandler {
    ...
    public static void notifyExploitDetected() {
        ...
        // Header with cooldown: ⛔ [Incognito] Translation exploit detected!
        if (now - lastHeaderTime >= HEADER_COOLDOWN_MS) {
            ...
            if (IncognitoConfig.getInstance().shouldShowAlerts()) {
            ...
            // Log with source
            if (IncognitoConfig.getInstance().isLogDetections()) {
                Incognito.LOGGER.info("[Incognito] Translation exploit detected via {}", 
    ...
    public static void logDetection(String keyName, String originalValue, String spoofedValue) {
        ...
        PrivacyLogger.ExploitSource source = ExploitContext.getSource();
        Incognito.LOGGER.info("[Translation:{}] '{}' '{}' → '{}'", 
    ...
    private static boolean shouldProcess() {
        return IncognitoConfig.getInstance().shouldShowAlerts() 
            || IncognitoConfig.getInstance().isLogDetections();
    }
